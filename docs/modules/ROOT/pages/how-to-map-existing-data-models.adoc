...
The business case translates into the following *use case specification*, which is instantiated from the https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/use-cases.html#sec:uc21[general UC2.1 description]:
...
This section provides general guidelines to address use case https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/use-cases.html#sec:uc12:~:text=Use-,Case,-UC2.1%3A%20Map%20an[UC2.1], mapping an ontology to a Core Vocabulary.

In this section we adopt the definitions from the ontology matching handbook [https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#ref:54[om]] for the following concepts:
...
image::images/UC21guidelineOverview.png[]
...
This section provides an overview of the guideline, which will be demonstrated in the https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/map-existing-ontology-to-vocabulary.html[tutorial ]section where we map the Core Business Vocabulary to Schema.org. The reader familiar with ontology matching could skim or skip it and proceed swiftly to the tutorial section.
...
Ontology alignments are often expressed in Alignment Format (AF) [https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#ref:78[af]] or Expressive and Declarative Ontology Alignment Language (EDOAL) [https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#ref:61[edoal], al-api].
...
Numerous well-established ontology matching algorithms have been extensively reviewed in the literature (for a review and in-depth analysis, see [https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#:~:text=%5B-,OteroEtAl15,-%5D%20Otero%2DCerdeira[om-lr], https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#ref:54[om]]). The main categories of ontology matching techniques are listed below in the order of their relevance to this handbook:
...
*Tools*: Tools such as https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#:~:text=%5B-,silk,-%5D%20System%20for[Silk] [silk], https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#:~:text=%5B-,LIMES,-%5D%20Ngomo%2C%20Axel[LIMES] [limes], and https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/references.html#:~:text=%5B-,LogMap,-%5D%20Jim%C3%A9nez%2DRuiz[LogMap] [logmap] may be used in this phase.
...
image::images/alignmentDiagram.png[]
...
*Tools*: Tools such as https://semiceu.github.io/core-vocs-handbook/public-review/references.html#ref:18:~:text=%5B-,vocbench,-%5D%20VocBench.%20Available[VocBench3] [vocbench] can be used in this phase, or generic office tools, such as MS Excel, Google Sheets spreadsheets, or a LibreOffice spreadsheet.
...
This tutorial demonstrates how to map http://Schema.org[Schema.org] to the https://github.com/SEMICeu/Core-Business-Vocabulary[Core Business Vocabulary] *(CBV)*, addressing *Use Case* https://github.com/SEMICeu/core-vocs-handbook/blob/develop/docs/modules/ROOT/pages/use-cases.adoc#:~:text=Use%20Case%20UC2.1%3A%20Map%20an%20existing%20Ontology%20to%20a%20Core%20Vocabulary[*UC2.1]*. By following the https://github.com/SEMICeu/core-vocs-handbook/blob/develop/docs/modules/ROOT/pages/map-an-existing-model-guideline.adoc#:~:text=Guidelines%20for%20how%20to%20map%20existing%20data%20models[map an existing model]** ***methodology*, +
...
For this tutorial, we aim to map Schema.org[Schema.org] to the Core Business Vocabulary (CBV), enabling data interoperability.
...
* *Purpose*: Enable semantic interoperability between business-related data models using CBV and Schema.org[Schema.org].
...
In this tutorial, we use http://limes.sf.net[LIMES] [limes] to automate link discovery in the mapping process between the *CBV* and *Schema.org*. While other tools could also be used, LIMES was selected for its simplicity and efficiency in performing lexical similarity-based alignments.
======= Set Up Data Sources
...
* Source entity: a URI from the target ontology (e.g., Schema.org[Schema.org]).
...
Each candidate correspondence is checked for correctness and relevance. For CBV and Schema.org, we use a manual inspection by ontology engineers first, which includes cross-referencing documentation or definitions. CBV’s +LegalEntity+ is “_A self-employed person, company, or organisation that has legal rights and obligations_” and Schema.org’s +Organization+ is “_An organization such as a school, NGO, corporation, club, etc._”. For +Address+, the descriptions are as follows: _“A spatial object that in a human-readable way identifies a fixed location”_ with a usage note indicating it to be understood as a postal address, and “_The mailing address_”, respectively.
...
For our running example, CBV’s +LegalEntity+ is almost the same as Schema.org’s +Organization+, but the latter does not have the “legal rights” constraint, and therefore, the appropriate semantic relation is that of _subsumption_. For the respective addresses, while CBV’s imported +Address+’ definition is broader than +PostalAddress+, and therefore suggesting a subsumption alignment as well, taking into account CBV’s usage note, it can be an _equivalence_ alignment.
...
* Further information on a mapping justification, which reuses the Simple Standard for Sharing Ontological Mapping https://w3id.org/sssom/[SSSOM ]that, in turn, reuses the Semantic Mapping Vocabulary https://mapping-commons.github.io/semantic-mapping-vocabulary/[SEMAPV] for the mapping justification values. Example output of adding an alignment between https://semiceu.github.io/Core-Business-Vocabulary/releases/2.00/#Legal%20Entity[ legal:LegalEntity] and https://schema.org/Organization[schema:Organization]:
...
This business case translates into the following *use case specification*, which is instantiated from the https://meaningfy-ws.github.io/semic-docs-staging/core-vocs-handbook/develop/use-cases.html#sec:uc22[general UC2.2 description]:
...
This section provides detailed instructions for addressing use case https://github.com/SEMICeu/core-vocs-handbook/blob/develop/docs/modules/ROOT/pages/use-cases.adoc#:~:text=Use%20Case%20UC2.2%3A%20Map%20an%20existing%20XSD%20Schema%20to%20a%20Core%20Vocabulary[UC2.2].
...
a| complexType&#124;group&#124;attributeGroup
|owl:Class
|Relationships and contextual restrictions packageR

|complexType//element
|owl:Restriction
|Filler of a relationship in that axiom/context

a |extension@base &#124; restriction@base
|rdfs:subClassOf
...
This approach results in a basic 1:1 direct alignment, which lacks contextual depth and specificity. Another approach would be to embed semantic annotations into XSD schemas using standards such as Semantic Annotations for WSDL and XML Schema (SAWSDL) [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Bsawsdl%5D%20SAWSDL.%20Available%20at%3A%20https%3A//www.w3.org/TR/sawsdl/%23annotateXSD[sawsdl]] using the *+sawsdl:modelReference+* attribute to capture this mapping. Such an approach is appropriate in the context of WSDL services.

====== Option 2b. Map a XSD to the XSD of a CV
...
After completing either Phase 2a or 2b, one may proceed to Phase 3, if applicable, else jump to Phase 4.
...
Both vocabulary and profile mappings are typically crafted and validated by domain experts, data-savvy business stakeholders, and in collaboration with semantic engineers and CV experts.
...
Several technology options are available [ml-lr] to represent these mappings technically, such as XSLT [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Bxslt%5D%20XSL%20Transformations%20(XSLT)%20Version%203.0.%20Available%20at%3A%20https%3A//www.w3.org/TR/xslt%2D30/[xslt]], RML [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Brml%5D%20RDF%20Mapping%20Language%20(RML).%20Available%20at%3A%20https%3A//rml.io/specs/rml/[rml]] (extending R2RML), and https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=sparql%2Danything%5D%20SPARQL%2DAnything.%20Available%20at%3A%20https%3A//github[SPARQLAnything]. RML allows for the representation of mappings from heterogeneous data formats, such as XML, JSON, relational databases, and CSV into RDF. The mapping rules can be expressed in Turtle RML or the YARRRML [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Byarrrml%5D%20YARRRML.%20Available%20at%3A%20https%3A//rml.io/yarrrml/spec[yarrrml]] dialect, a user-friendly text-based format based on YAML, making the mappings accessible to both machines and humans. RML is well-supported by robust implementations such as RMLMapper [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=rml%2Dmapper%5D%20RML%20Mapper.%20Available%20at%3A%20https%3A//github.com/RMLio/rmlmapper%2Djava[rml-map]] and RMLStreamer [rml-stream], which provide robust platforms for executing these mappings. RMLMapper is adept at handling batch processing of data and RMLStreamer suits streaming data scenarios, where data needs to be processed in real-time.

Provided one has mastered RML along with XML technologies such as XSD, XPath, and XQuery to implement the mappings effectively [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Brml%2Dgen%5D%20Generate%20RDF%20from%20an%20XML%20file.%20Available%20at%3A%20https%3A//rml.io/docs/rml/tutorials/xml/[rml-gen]], the development of the technical mapping rules is straightforward thanks to the conceptual mapping output from Phase 3.1. RML mapping statements are created for each class of the target ontology coupled with the property-object mapping statements specific to that class.
...
SHACL is a suitable choice to ensure adherence to data standards and interoperability requirements. This form of validation is independent of the way in which data mappings are constructed, focusing instead on whether the generated data conforms to established semantic models. It provides a high-level assurance that data structures and content meet the specifications.
...
** *Technical Mapping Files*: This contains all the mapping code files ([https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Bxslt%5D%20XSL%20Transformations%20(XSLT)%20Version%203.0.%20Available%20at%3A%20https%3A//www.w3.org/TR/xslt%2D30/[xslt]], [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Brml%5D%20RDF%20Mapping%20Language%20(RML).%20Available%20at%3A%20https%3A//rml.io/specs/rml/[rml]], [https://semiceu.github.io/core-vocs-handbook/develop/references.html#:~:text=%5Bsparql%2Danything%5D%20SPARQL%2DAnything.%20Available%20at%3A%20https%3A//github.com/SPARQL%2DAnything/sparql.anything[SPARQLAnything]], depending on the chosen mapping technology) for data transformation.
...
This maps the XML element *+<LegalName>+* to the RDF property** ***+legal:legalName+***.**
