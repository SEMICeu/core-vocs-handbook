...
*Tools*: Tools such as SILK [xref:references.adoc#ref-62[silk]], LIMES [xref:references.adoc#ref-67[limes]], and LogMap [xref:references.adoc#ref-83[logmap]] may be used in this phase.
...
*Rendering* translates the mapping in a machine-readable format so that it can be interpreted and executed by software agents. Typically, this is a straight-forward export of the alignment statements from the editing tool or the materialisation of the mapping in a triple store, using a common format, such as Alignment Format [xref:references.adoc#ref-78[af]], EDOAL [xref:references.adoc#ref-61[edoal], Simple Standard for Sharing Ontological Mapping (SSSOM) [xref:references.adoc#ref-79[sssom]], and the Semantic Mapping Vocabulary (SEMAPV) [semapv] for the mapping justification values. Multiple renderings may be created from the same alignment, accommodating the need for various formalisms.

*Tools*: Tools such as https://semiceu.github.io/core-vocs-handbook/public-review/references.html#ref:18:~:text=%5B-,vocbench,-%5D%20VocBench.%20Available[VocBench3] [xref:references.adoc#ref-63[vocbench]] can be used in this phase, or generic office tools, such as MS Excel, Google Sheets spreadsheets, or a LibreOffice spreadsheet.  +
...
This tutorial demonstrates how to map http://Schema.org[Schema.org] to the https://github.com/SEMICeu/Core-Business-Vocabulary[Core Business Vocabulary] *(CBV)*, addressing *Use Case* link:./introduction.adoc#sec:uc21[UC2.1]. By following the link:./how-to-map-existing-data-models.adoc#_guidelines_on_how_to_map_an_existing_conceptual_model_to_a_core_vocabulary[map an existing model] *methodology*, +
