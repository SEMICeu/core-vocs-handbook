...
This section provides general guidelines to address use case xref:use-cases.adoc#sec:uc21[UC2.1], matching an ontology to a core vocabulary The term ‘ontology’ is used loosely, and may also be similar artefacts, such as OWL-formalised conceptual data models or structured controlled vocabularies.  
...
This section provides an overview of the guideline, which will be demonstrated in the xref:map-an-exiting-model-tutorialUC21.adoc[tutorial] section where we map the Core Business Vocabulary to Schema.org. 
...
*Tools:* Tools such as xref:references.adoc#ref:62[Silk], xref:references.adoc#ref:76[LIMES], and xref:references.adoc#ref:77[LogMap]LogMap may be used in this stage.
