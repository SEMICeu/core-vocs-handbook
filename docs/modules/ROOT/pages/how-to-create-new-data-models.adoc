...
A context can be defined per class, allowing property names such as [.custom-highlight]#name# or [.concept]#description# to be reused within each class-specific scope. This produces cleaner and more human-readable JSON but can be more complex to design and maintain. Scoped contexts often require explicit `@type` declarations or additional range indicators to ensure that the correct mappings are applied during JSON-LD expansion.
