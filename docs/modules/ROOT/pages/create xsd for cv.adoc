...
Creating an *XSD schema* for the *Core Vocabulary* involves defining the structure,
data types, and relationships for the elements of the CBV, ensuring interoperability between systems.
This tutorial follows the xref:guideline-create-xsd-for-cv.adoc[methodology]
outlined for *Use Case* xref:UC1.1-description.adoc#sec:uc11[*UC1.1]*: "Create a New XSD Schema," showing how to design and create an XSD schema that integrates terms from the
https://interoperable-europe.ec.europa.eu/collection/semic-support-centre/solution/e-government-core-vocabularies/core-business-vocabulary[*Core Business Vocabulary*] (CBV).
...
To ensure schema compliance, the *Schematron rules* provide automated checks. These rules cover key aspects such as type definitions, element declarations, metadata, and more. The detailed list of rules can be found 
https://github.com/SEMICeu/XML-schema/blob/main/rules/rules_schematron.sch[here].
