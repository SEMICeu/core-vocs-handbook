...
For first-time readers of this Handbook, we recommend starting with the remainder of this section, where <<sec:interoperability, interoperability>> is introduced, the role of  <<sec:Core_Vocabulary,Core Vocabularies>> is explained, and the important <<sec:use-cases, use cases>> are summarised.
...
****
*Sidebar 1: Interoperability as enabler of benefits.* In the context of the public sector, interoperability functions as an enabler of a number of benefits. Among others, it:
...
****
*Sidebar 2: Reasons for interoperability in public administrations.* There are two key reasons to seek interoperability in public administrations:
...
We differentiate between use cases focussed on the creation of _new_ artefacts and those involving the mapping of _existing_ artefacts to Core Vocabularies. For better clarity, we numbered the use cases and organised them into two diagrams, one for the primary scenarios (see Figure 2, below) and the other for depicting the additional ones. The former are addressed in the main part of this handbook, whereas the latter are listed in the  xref:Appendix.adoc[Appendix] and may be elaborated on at a later date if there is a demand from the community.
...
.Primary use cases covered in this handbook
