...
We differentiate between use cases focussed on the creation of _new_ artefacts and those involving the mapping of _existing_ artefacts to Core Vocabularies. For better clarity, we numbered the use cases and organised them into two diagrams, one for the primary scenarios (see Figure 1, below) and the other for depicting the additional ones. The former are addressed in the main part of this handbook, whereas the latter are listed in the  xref:Appendix.adoc[Appendix] and may be elaborated on at a later date if there is a demand from the community.
